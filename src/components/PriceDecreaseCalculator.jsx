...
const PriceDecreaseCalculator = () => {
	...
	const calculatePriceDecrease = async () => {
		setLoading(true);
		setResult("Загрузка...\n");
		...
		try {
			...
			if (isNaN(desiredPrice) || desiredPrice >= currentPrice) {
				setResult("Целевая цена должна быть ниже текущей");
				return;
			}
			...
			output += `                 1 ${token1Name} = ${formatFullDecimal(
				1 / currentPrice,
				18
			)} ${token0Name}\n`;
			output += `🎯 Целевая цена: 1 ${token0Name} = ${formatFullDecimal(
				desiredPrice,
				18
			)} ${token1Name}\n`;
			output += `                 1 ${token1Name} = ${formatFullDecimal(
			...
			const simulate = async (amount) => {
				const parsedAmount = ethers.utils.parseUnits(amount.toString(), decimals0);
				const params = {
					tokenIn: token0,
					tokenOut: token1,
					fee: parseInt(fee),
			...
			while (min < max && iterations < 30) {
				...
				if (!simulatedPrice) {
					output += `🔍 Swap try №${iterations + 1}: ${testAmount.toFixed(
						0
					)} ${token0Name} → ошибка (симуляция не удалась)\n`;
					max = testAmount;
					iterations++;
					continue;
				}
				...
				output += `🔍 Swap try №${iterations + 1}: ${testAmount.toFixed(
					0
				)} ${token0Name} → цена ${formatFullDecimal(
					simulatedPrice,
					18
				)} (δ = ${errorPercent.toFixed(3)}%)\n`;
				if (errorPercent < bestError) {
					bestAmount = testAmount;
					bestError = errorPercent;
				}
				if (errorPercent < 0.1) break;
	...
	return (
		<div className='calculator'>
			<h2>Расчёт понижения цены (Price ↓)</h2>
			<input
				type='text'
			...
			<button onClick={calculatePriceDecrease} disabled={loading}>
...
export default PriceDecreaseCalculator;
